...
def get_parliament_periods_to_database(cursor):
    ...
    for period in json_data['data']:
        insert_command = "INSERT INTO parliament_periods VALUES ({}, {}, '{}', {}, {}, {})".format(
            ...
            "'"+period['start_date_period'].replace("'", "''")+"'" if period['start_date_period'] is not None else 'null',
            "'"+period['end_date_period'].replace("'", "''")+"'" if period['end_date_period'] is not None else 'null'
            )
        print(insert_command)
        cursor.execute(insert_command)
        ...
        for candidacy_or_mandate in json_data['data']:
            ...
            if candidacy_or_mandate['type'] == "mandate":
                ...
                if fraktion != 'fraktionslos':
                    cursor.execute(f"SELECT id FROM parties WHERE '%s' like lower(parties.short_name) || '%s'"%( fraktion.lower(), "%"))
                    res = cursor.fetchall()
...
def get_parties_to_database(cursor):
    ...
    for party in json_data['data']:
        cursor.execute(f"INSERT INTO parties VALUES (%d, '%s', '%s')"%(party['id'], party['full_name'], party['short_name']))
